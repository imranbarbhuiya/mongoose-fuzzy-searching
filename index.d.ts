...
//   type MongooseQuery<T, any> = mongoose.Query<T, any>;
...
//     fuzzySearch(...args: StaticFuzzyParameters): MongooseQuery<T, any>;
//   }

//   export interface MongoosePluginModel<T extends MongooseDocument>
//     extends mongoose.Model<T, MongooseQueryHelpers<T>> {
//     fuzzySearch(query: Query): MongooseQuery<T, any>;
//     fuzzySearch(query: Query, options: MongooseCriteria): MongooseQuery<T, any>;
//     fuzzySearch(query: Query, callback: MongooseCallback): MongooseQuery<T, any>;
//     fuzzySearch(
//       query: Query,
//       options: MongooseCriteria,
//       callback: MongooseCallback,
...
declare module "mongoose" {
  ...
  type MongooseCriteria = FilterQuery<any> | Query<any, any>;
  type MongooseCallback = (err: any, res: any[]) => void;

  export interface MongooseQueryHelpers<T extends Document> {
    fuzzySearch(query: PluginQuery): Query<T, any>;
    fuzzySearch(query: PluginQuery, options: MongooseCriteria): Query<T, any>;
    fuzzySearch(query: PluginQuery, cb: MongooseCallback): Query<T, any>;
    fuzzySearch(
      query: PluginQuery,
      options: MongooseCriteria,
      cb: MongooseCallback
...
declare module "mongoose-fuzzy-searching" {
  ...
  const _: (
    ...
    Options?: mongoose.FuzzySearchOptions
  ) => void;
