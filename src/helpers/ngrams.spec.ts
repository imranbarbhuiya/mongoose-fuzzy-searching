import { makeNGrams, nGrams } from "./ngrams";
...
describe("nGrams", () => {
  ...
  describe("with default values", () => {
    ...
    it('should be equal to ["a"] when the given text is `a`', () => {
      expect(nGrams("a", options.minSize, options.prefixOnly)).toStrictEqual([
        "a",
      ]);
    });

    it('should be equal to `["aa", "aaa"]` when the given text is `aaa`', () => {
      expect(nGrams("aaa", options.minSize, options.prefixOnly)).toStrictEqual([
        "aa",
        "aaa",
      ]);
    });
    ...
    it('should be equal to `["23", "12", "123"]` when the given text is 123 (as a number)', () => {
      expect(nGrams("123", options.minSize, options.prefixOnly)).toStrictEqual([
        "23",
        "12",
        "123",
      ]);
  ...
  describe("with custom `prefixOnly`", () => {
    it('should return `["aa", "aaa", "aaaa", "aaaaa"]` when the minSize is 3 and prefixOnly to true', () => {
      expect(nGrams("aaaaa", options.minSize, true)).toStrictEqual([
        "aa",
        "aaa",
        "aaaa",
        "aaaaa",
